...
//                 {isDownloaded
//                   ? " "
//                   : "Download PDF by clicking this and send it to our Whatsapp Number after confirmation"}
//               </div>
//             )
//           }
//         </PDFDownloadLink>
...
const ConfirmListPage = ({
  ...
  const handleConfirmOrder = async () => {
    ...
    const formUrl =
      "https://script.google.com/macros/s/AKfycbw5GhqEY7at54dkosb4LCMID9_Q3qWew8hr20w8yj7MBpVl8W2lZgS7Rsuqn9guslS/exec?sheet=Hari Haran Trader's Customer";
  ...
  const handleDownloadAndSend = async () => {
    if (selectedItemsPdf.length > 0) {
      ...
      const doc = <MyDocument
        ...
        totalRate={totalRate}
        selectedItemsPdf={selectedItemsPdf}
      ...
      a.href = url;
      a.download = "Ordered-List.pdf";
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);

      // Open WhatsApp with a pre-filled message
      const phoneNumber = '7339437623';
      const message = `*Dear HariHaran Trader,*\n\nI have attached my order list PDF. Please review and confirm.\n\n*Customer Name:* ${customerName}\n*Customer Number:* ${customerNumber}\n\nThank you!`;
    ...
    return (
      <Document>
        <Page size="A4" style={styles.page}>
          ...
          <Text style={styles.orderInfo}>Mobile Number: 9444324237</Text>
            ...
            {Object.keys(groupedItems).map((category, categoryIndex) => (
              <View key={categoryIndex}>
                {groupedItems[category].map((item, itemIndex) => {
                  serialNumberPdf++;
                  return (
                    <View key={`${categoryIndex}-${itemIndex}`} style={styles.tableRow}>
                      ...
                      <Text
                        style={[
                          styles.tableCell,
                          { flex: 1, textAlign: "left", fontFamily: "Noto Sans Tamil" },
                        ]}
                      >
                        {item.tamilName}
                      </Text>
          ...
          <Text style={[styles.customerInfoText, { marginTop: 6 }]}>
            Customer Number : {customerNumber}
          </Text>
          <Text style={[styles.customerInfoText, { marginTop: 6 }]}>
            Customer Address : {customerAddress}
          ...
          <Text style={[styles.customerInfoText, { marginTop: 15 }]}>
            Overall Total Amount : {totalRate}
          </Text>
        </Page>
      </Document>
  ...
  return (
    <div>
      ...
      {isConfirmed && (
        <div className="download-and-whatsapp-container">
          <button
            className="whatsapp-button"
            onClick={handleDownloadAndSend}
            disabled={loadingPdf}
          >
            {loadingPdf ? "Generating PDF..." : "Download & Send to WhatsApp"}
          </button>
        </div>
      )}
